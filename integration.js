...
function _setupRegexBlocklists(options) {
  ...
  if (
    options.emailBlocklistRegex !== previousEmailRegexAsString &&
    options.emailBlocklistRegex.length === 0
  ) {
    ...
    if (options.emailBlocklistRegex !== previousEmailRegexAsString) {
      ...
      Logger.debug(
        { emailBlocklistRegex: previousEmailRegexAsString },
        'Modifying Email Blocklist Regex'
      );
...
function _lookupEntity(entityObj, options, cb) {
  ...
  requestWithDefaults(requestOptions, function (err, response, body) {
    ...
    const totalPasteBins = pastebins.length;
    // Only return first 20 paste bins
    const slicedPastebins = pastebins.slice(0, 20);
    ...
    cb(null, {
      ...
      data: {
        ...
        summary: ['Total Pastebin Dumps: ' + totalPasteBins],
        // Data that you want to pass back to the notification window details block
        details: {
          pastebins: slicedPastebins,
          totalPasteBins,
          additionalPasteBins: totalPasteBins - 20 < 0 ? 0 : totalPasteBins - 20
        }
      }
    });
  });
...
function getPastebins(options, body) {
  ...
  return fp.flow(
    ...
    fp.filter(
      ({ time }) =>
        ...
        moment.utc(new Date()).diff(moment(time), 'days', false) <= DAYS_TO_LOOK_BACK
    )
  )(body);
}
...
function validateOptions(userOptions, cb) {
  ...
  if (
    typeof userOptions.emailBlocklistRegex.value === 'string' &&
    userOptions.emailBlocklistRegex.value.length > 0
  ) {
    try {
